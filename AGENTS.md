...
- 每个复杂事项至少维护 `plan.md`、`case.md`、`skill_outputs.md`、`analysis.md`、`advice.md`。专业报告 Markdown（.md）只在用户要求报告文件、阶段交付或正式归档时生成；专业报告 PDF（.pdf）只在用户明确要求 PDF 时生成。
...
- 未请求 PDF 时，`PDF Gate` 应标记为 `skipped / not requested`，不影响会话汇总结论或专业报告 Markdown（.md）的阶段报告状态。若用户已请求 PDF，更新专业报告 Markdown（.md）后必须重新生成同源专业报告 PDF（.pdf）并做基础可读性检查；若不能重渲染或检查不通过，不得把旧 PDF 标为本轮 ready。
- 面向用户、专业报告 Markdown（.md）和专业报告 PDF（.pdf）的术语必须统一；统一口径以 `docs/WORKFLOW.md` 的“术语与输出口径”为准。
...
13. 用户要求正式报告文件时，调用最终汇总规则，逐项读取所有工作文件，生成专业报告 Markdown（.md）。
14. 用户明确要求 PDF 时，将专业报告 Markdown（.md）渲染为 styled HTML、DOCX、PDF-native 对象或宿主支持的富文本版式后再导出 PDF；可用根目录渲染器时，调用 `tools/render_report_pdf.py <事项文件夹>`，输入目录即输出目录，只在事项文件夹生成专业报告 PDF（.pdf），不复制渲染脚本。可用 styled HTML 路径时，优先引用或内联 `assets/legal-report.css`。
