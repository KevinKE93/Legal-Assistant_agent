...
本文件定义每个阶段 skill 的触发条件、必须落盘的文件、`skill_outputs.md` 记忆字段和最终报告章节映射。每个 `skills/*/SKILL.md` 只保留阶段操作细节；本文件提供跨技能统一约束。哪些 skill 必跑、可选或可跳过，以 `CAPABILITIES.md` 的事项类型矩阵为准。案件工作台文件的生成和更新规则以 `CASE_WORKBENCH.md` 为准。
...
复杂纠纷、仲裁/诉讼、对方已有主张、返还/赔偿/解除等事项，必须同时遵循 `LEGAL_REASONING.md`：争点不能只列清单，必须形成母命题、条件命题、反制命题、推断链条和法条适用边界。相关发现写入 `analysis.md`，并在最终报告中对应“争点深挖与推断链”“法条适用边界”章节。

案件工作台事项还必须遵循 `CASE_WORKBENCH.md`：初次复杂纠纷优先生成 `case_dashboard.md` 和 `consultation_note.md`；深度阶段按需生成 `case_package.md`、`pleading_framework.md`、`hearing_playbook.md`；复盘阶段生成 `review_delta.md`。这些文件不是额外模板，而是各 skill 发现的工作台表达。
...
| `case_intake_issue_map` | 纠纷事实零散、需要形成案件地图 | `case.md`、`analysis.md`、`case_dashboard.md`、`consultation_note.md`、`skill_outputs.md` | 事项地图、案件驾驶舱与争点总览 |
...
- 专业报告必须吸收 `Execution Index` 中每个已执行 skill 的关键发现，并转化为事实、证据、争点、法律依据、风险或行动建议。
- 专业报告不得展示必跑和条件必跑 skill 的执行表；必跑或条件必跑 skill 未执行、`pending` 或 `blocked` 时，报告状态不能标记为 `complete`，并应根据影响标记为 `draft` 或 `incomplete`，同时在“分析范围与可靠性说明”中写成读者能理解的限制。
...
|---|---|---|---|
| `case_dashboard.md` | `privacy_scope_guard`、`case_intake_issue_map`、已完成的证据/来源/对方/裁判视角 skill | 一句话地图、程序状态、三个胜败关键、争点树、证明责任、证据缺口、对方打法、来源状态、可信度 | 下一步五个动作、输出分流、长篇法条摘录 |
| `consultation_note.md` | `case_intake_issue_map`、`strategy_risk_action`、`case_reference_research` | 当前能判断什么、不能判断什么、风险、补证材料、禁忌动作、下一次协作清单 | 内部 gate、skill 执行索引、过度确定的胜率 |
| `case_package.md` | 证据、要件、来源、对方视角、裁判视角、策略和草拟类 skill | 事实、证据、争点、请求权基础、来源边界、攻防、策略、文书/庭审索引 | 对外报告式包装、无依据的最终结论 |
...
| `hearing_playbook.md` | `hearing_prep`、`opponent_perspective`、`judge_perspective`、`admission_question_design` | 庭审目标、举证顺序、质证、合法发问、裁判者追问、安全回答原则 | 诱导虚假陈述、非法取证或威胁性话术 |
| `review_delta.md` | `review_learning_loop` | 新旧差异、受影响事实/争点、变化判断、需更新文件、需重跑 skill | 从头重写却不说明变化 |

这些文件每次更新都要在 `skill_outputs.md` 中记录来源 skill、读取文件、更新内容和最终报告章节映射。
