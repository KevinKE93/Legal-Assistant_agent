...
4. 输出本地化命名的专业报告 `.md` 和同名 `.pdf`。
5. 对 PDF 做基本质量检查；若不合格，明确标记 blocked。
...
读取 `skill_outputs.md`，建立 skill 覆盖表：

| Skill | 是否执行 | 关键发现 | 待补问题 | 对应报告章节 | 是否已纳入 |
|---|---|---|---|---|---|
...
17. 禁忌动作与表达风险。
18. 待补信息。
19. 附录：工作文件覆盖表、Skill 覆盖表、报告生成质量检查。
...
将同一内容导出为同名 PDF。优先使用支持 CJK 字体的宿主导出能力；其次使用可确认中文字体可读的 Markdown/PDF 工具；再其次使用浏览器打印为 PDF。
